...
class Email_via_Gmail():
    def __init__(self, login_info, address_to, subject, text):
    ...
    def send_an_email(self):
        ...
        msg['Subject'] = self.title  # email title
        msg['From'] = self.ID  # send-from email address
        msg['To'] = self.mail_to
        print(msg.as_string())

        s = smtplib.SMTP('smtp.gmail.com', 587)  # Gmail's smtp addresss and port number
        s.starttls()
        s.login(self.ID, self.PW)
        s.sendmail(self.ID, self.mail_to, msg.as_string())
        print(f'Email [{self.title}] sent to {self.mail_to} successfully.')
        s.quit()
